...
- **Uploaded image 404s at `/media/...`**: `DEFAULT_FILE_STORAGE` is unconditionally Cloudinary (`vote_fund/settings.py`), so this should not normally happen — a real `.../media/...` URL means the image was actually saved to local disk instead of Cloudinary, which means Cloudinary isn't truly active for that container. Check, in order:
  1. `docker compose exec web python -c "import os; print(os.getenv('CLOUDINARY_CLOUD_NAME'), os.getenv('CLOUDINARY_API_KEY'))"` — if either is empty/`None`, the `.env` the container is running with is missing them (double-check `env_file: .env` is pointing at the file you think it is, and that it isn't a stale `.env` baked into an old image layer).
  2. `docker compose exec web python manage.py shell -c "from django.core.files.storage import default_storage; print(default_storage.__class__)"` — should print a Cloudinary storage class, not `FileSystemStorage`.
  3. `docker compose exec web python manage.py shell -c "from voting.models import Event; e = Event.objects.get(pk=<id>); print(e.event_image, e.event_image.url)"` — confirms what URL the affected row's field actually resolves to right now.
  4. If Cloudinary comes back correctly configured but old rows still show local paths, those rows were saved before Cloudinary was correctly wired up (stale data) — re-upload the image through the app/admin to move it to Cloudinary.
  5. Also confirm `DEBUG=False` in production — `DEBUG=True` is what makes Django serve `/media/` locally at all (`vote_fund/urls.py`) and is what's rendering the verbose debug 404 page (which itself leaks internals and shouldn't be shown to real users).
  In the meantime, `docker-compose.yml`'s `media_data` volume (mounted at `/app/media`) prevents whatever is already on local disk from being wiped by a restart, but it doesn't redirect future uploads to Cloudinary — that depends on the container actually having valid `CLOUDINARY_*` env vars.
